...
const CandidateList: React.FC = () => {
  ...
  const calculateJobMatchScore = (candidate: CandidateProfile, job: Job): EnhancedCandidate => {
  ...
  const filteredCandidates = React.useMemo(() => {
    ...
    return filtered.sort((a, b) => {
      ...
      if (sortOption === "score_asc") {
        const scoreA = selectedJob !== "all" ? (a.jobMatchScore || 0) : (calculateAverageScore(a.skills) || 0);
        const scoreB = selectedJob !== "all" ? (b.jobMatchScore || 0) : (calculateAverageScore(b.skills) || 0);
      ...
      {selectedJob !== "all" && topCandidateForJob && (
        <div className="card highlight-card" style={{ margin: '0 2rem 2rem 2rem' }}>
          ...
          <div className="detail">
            ...
            ({topCandidateForJob.matchedSkills}/{topCandidateForJob.totalJobSkills} skills)
          </div>
          <button
                  ...
                  >
                    ...
                    <td>
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                        <span
                          style={{
                            color: category ? category.color : getScoreColor(score),
                            fontWeight: "bold",
                          }}
                        >
                          {formatScore(score)}{selectedJob !== "all" ? "%" : ""}
                        </span>
                        {category && (
                    ...
                    <td>
                      {selectedJob !== "all" && enhancedCandidate.jobMatchScore !== undefined ? (
                        <span>
                          {enhancedCandidate.matchedSkills || 0}/{enhancedCandidate.totalJobSkills || 0}
                          {enhancedCandidate.missingSkills && enhancedCandidate.missingSkills.length > 0 && (
                            <div style={{ fontSize: '11px', color: '#ef4444' }}>
                              Missing: {enhancedCandidate.missingSkills.slice(0, 2).join(', ')}
